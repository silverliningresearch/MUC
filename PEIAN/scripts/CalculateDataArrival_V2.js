...
function PreparaArrivalData() {
  ...
  {  
    ...
    quota_data = [
      ...
      { "Location": "T1-C", "Exit": 3, "Belt" : 5, "Landside" :3, "Total" : 10},
      { "Location": "T1-D", "Exit": 4, "Belt" : 8, "Landside" :4, "Total" : 15},
      { "Location": "T1-E", "Exit": 4, "Belt" : 8, "Landside" :4, "Total" : 15},
  ...
  for (i = 0; i < location_count_data.length; i++) {
    ...
    if (result.Month + "-" + result.Year == currentMonth)
    {
      for (j = 0; j < quota_data.length; j++) {
        ...
        if (result.Location == "T1-A/E03")
        {
          result.Location = "T1-A";
        }

        if (result.Location == "T1-A/E04")
        {
          result.Location = "T1-A";
        }
        ...
        if (item.Location == result.Location)
        {
          ...
          if (result.Exit_belt == "Exit") 
          {
            quota_data[j].Exit = quota_data[j].Exit - result.completed_interviews;
            quota_data[6].Exit = quota_data[6].Exit - result.completed_interviews;

          }
          if (result.Exit_belt == "Baggage belts") 
          {
            quota_data[j].Belt =  quota_data[j].Belt - result.completed_interviews;
            quota_data[6].Belt = quota_data[6].Belt - result.completed_interviews;
          }        
          if (result.Exit_belt == "Landside") 
          {
            quota_data[j].Landside = quota_data[j].Landside -  result.completed_interviews;
            quota_data[6].Landside = quota_data[6].Landside - result.completed_interviews;
          }    
        }
